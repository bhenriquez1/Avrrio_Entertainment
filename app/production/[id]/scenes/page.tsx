...
export default function ScenesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function createScene(event: React.FormEvent) {
    ...
    setSaving(true);
    const seasonId=searchParams.get("season")||undefined, episodeId=searchParams.get("episode")||undefined;
    const created = await saveScene(uid, productionId, { title: title.trim(), storyContext: storyContext.trim(), action: action.trim(), visualDirection: visualDirection.trim(), act:act.trim(), storyBeat:storyBeat.trim(), seasonId, episodeId, sceneNumber:scenes.filter(s=>s.episodeId===episodeId).length+1, status: "draft", linkedCanonIds: canon.map((item) => item.id), productionJobId: null });
    setScenes((current) => [created, ...current]);
    setTitle(""); setStoryContext(""); setAction(""); setVisualDirection(""); setAct(""); setStoryBeat(""); setShowComposer(false); setSaving(false);
  }
  ...
  return <main className="min-h-full bg-[radial-gradient(circle_at_top_right,rgba(30,64,175,.14),transparent_36%)] p-8"><div className="mx-auto max-w-6xl">
    ...
    {showComposer && <form onSubmit={createScene} className="mt-6 rounded-2xl border border-cyan-200/15 bg-[#0b1122] p-6"><div className="flex justify-between"><h2 className="font-semibold text-white">Develop scene</h2><button type="button" onClick={() => setShowComposer(false)} className="text-slate-500">×</button></div><div className="mt-5 grid gap-4 md:grid-cols-2"><label className="text-xs text-slate-400">Scene title<input value={title} onChange={(event) => setTitle(event.target.value)} className="mt-1.5 w-full rounded-lg border border-blue-200/10 bg-black/20 px-3 py-2.5 text-sm text-white outline-none" placeholder="Samantha's dream — opening" /></label><label className="text-xs text-slate-400">Story position<input value={storyContext} onChange={(event) => setStoryContext(event.target.value)} className="mt-1.5 w-full rounded-lg border border-blue-200/10 bg-black/20 px-3 py-2.5 text-sm text-white outline-none" placeholder="Season 1 • Episode 1 • Scene 1" /></label><label className="text-xs text-slate-400">Act<input value={act} onChange={e=>setAct(e.target.value)} className="mt-1.5 w-full rounded-lg border border-blue-200/10 bg-black/20 px-3 py-2.5 text-sm text-white" placeholder="Act I"/></label><label className="text-xs text-slate-400">Story beat<input value={storyBeat} onChange={e=>setStoryBeat(e.target.value)} className="mt-1.5 w-full rounded-lg border border-blue-200/10 bg-black/20 px-3 py-2.5 text-sm text-white" placeholder="Inciting incident"/></label></div><label className="mt-4 block text-xs text-slate-400">Action and performance<textarea value={action} onChange={(event) => setAction(event.target.value)} rows={5} className="mt-1.5 w-full resize-none rounded-lg border border-blue-200/10 bg-black/20 px-3 py-2.5 text-sm text-white outline-none" placeholder="What happens in the scene? Include character intention and emotional beat." /></label><label className="mt-4 block text-xs text-slate-400">Visual direction<textarea value={visualDirection} onChange={(event) => setVisualDirection(event.target.value)} rows={3} className="mt-1.5 w-full resize-none rounded-lg border border-blue-200/10 bg-black/20 px-3 py-2.5 text-sm text-white outline-none" placeholder="Camera, composition, lighting, motion, environment…" /></label><div className="mt-4 flex items-center justify-between"><p className="text-[11px] text-cyan-200/50">Draft only. No generation or provider charge.</p><button disabled={saving || !title.trim() || !action.trim()} className="rounded-lg bg-white px-4 py-2 text-sm font-bold text-slate-950 disabled:opacity-30">{saving ? "Saving…" : "Save Scene"}</button></div></form>}
